  ...
  const [ozet, setOzet,] = useState([
{
 ...
 const uniqueSummary = Array.from(
  new Set(ozet.map(item => item.ozet))
).map(alinti => {
  return ozet.find(item => item.ozet === alinti);
});

  const [filteredOzet, setFilteredOzet] = useState(uniqueSummary);
  
  const handleSearch = () => {
    const filtered = uniqueSummary.filter(item =>
      item.ozet.toLowerCase().includes(query.toLowerCase()) ||
      item.kitapAdi.toLowerCase().includes(query.toLowerCase())
    );


    
    setFilteredOzet(filtered);
  };
  ...
  return (
   <div className="d-flex flex-column align-items-center min-vh-100 p-4">
      ...
      <Row className="mt-4 w-100 justify-content-center">
        {filteredOzet.length > 0 ? (
          filteredOzet.map((item, index) => (
